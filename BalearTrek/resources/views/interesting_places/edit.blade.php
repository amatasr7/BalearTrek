<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Editar Publicación') }} : {{ $interestingPlace->name }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-8">
                <form action="{{ route('interesting-places.update', $interestingPlace) }}" method="POST">
                    ...
                    <div class="mb-4">
                        <label for="name" class="block text-sm font-medium text-gray-700 mb-1">Títol</label>
                        <input type="text" name="name" id="name" value="{{ old('name', $interestingPlace->name) }}" 
                            class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    </div>

                    <div class="mb-4">
                        <label for="gps" class="block text-sm font-medium text-gray-700 mb-1">Coordenades GPS</label>
                        <input type="text" name="gps" id="gps" value="{{ old('gps', $interestingPlace->gps) }}" 
                            class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    </div>

                    <div class="mb-6">
                        <label for="place_type_id" class="block text-sm font-medium text-gray-700 mb-1">Tipus de Lloc</label>
                        <select name="place_type_id" id="place_type_id" 
                            class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            @foreach($types as $type)
                                <option value="{{ $type->id }}" {{ $interestingPlace->place_type_id == $type->id ? 'selected' : '' }}>
                                    {{ $type->name }}
                                </option>
                            @endforeach
                        </select>
